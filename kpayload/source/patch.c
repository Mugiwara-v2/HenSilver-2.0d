...
PAYLOAD_CODE int shellcore_fpkg_patch(void)
{
	...
	ret = proc_write_mem(ssc, (void *)(text_seg_base + enable_fpkg_patch), 8, "\xE9\x98\x00\x00\x00\x90\x90\x90", &n);
	if (ret)
		goto error;
	...
	/*ret = proc_write_mem(ssc, (void *)(text_seg_base + verify_keystone_patch), 4, "\x48\x31\xC0\xC3", &n);
	if (ret)
	{
		goto error;
	}

	 ret = proc_write_mem(ssc, (void *)(text_seg_base + save_mount_permision), 3, "\x31\xC0\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }*/
	 ...
	 // patch debug setting 
 	 /*ret = proc_write_mem(ssc, (void *)(text_seg_base + ssc_sceKernelIsAssistMode), 5, "\x31\xC0\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }*/
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sceSblACMgrIsAllowedSystemLevelDebugging1), 6, "\x48\xC7\xC0\x01\x00\x00\x00\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sceSblACMgrIsAllowedSystemLevelDebugging2), 6, "\x48\xC7\xC0\x01\x00\x00\x00\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sceSblACMgrIsAllowedSystemLevelDebugging3), 6, "\x48\xC7\xC0\x01\x00\x00\x00\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }

	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sys_rwx_map), 1, "\x07", &n);
	 if (ret)
	 {
		 goto error;
	 }

	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sys_rwx_map1), 1, "\x07", &n);
	 if (ret)
	 {
		 goto error;
	 }

	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sys_rwx_patch), 2, "\xEB\x04", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sys_vm_fault_patch), 6, "\x90\x90\x90\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sys_patchA), 2, "\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sys_patchB), 2, "\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 /*ret = proc_write_mem(ssc, (void *)(text_seg_base + setloginpatch), 5, "\x48\x31\xC0\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }*/
	 
	 /*ret = proc_write_mem(ssc, (void *)(text_seg_base + sys_setuid), 5, "\xB8\x00\x00\x00\x00", &n);
	 if (ret)
	 {
		 goto error;
	 }*/
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + ptrace_patches), 6, "\x90\x90\x90\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 	  
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + copyinstr_patches), 2, "\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + copyinstr2_patches), 3, "\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + copyin_patches), 2, "\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + copyin2_patches), 3, "\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + copyout_patches), 2, "\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + copyout2_patches), 3, "\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 /*ret = proc_write_mem(ssc, (void *)(text_seg_base + dynlib_dlsym_patches), 5, "\x90\xE9", &n);
	 if (ret)
	 {
		 goto error;
	 }*/
	 
	 /*ret = proc_write_mem(ssc, (void *)(text_seg_base + Panic), 5, "\x90\x90\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }*/
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base), 4, "\x7F\x45\x4C\x46", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + map_self1), 6, "\xB8\x01\x00\x00\x00\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + map_self2), 6, "\xB8\x01\x00\x00\x00\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + map_self3), 5, "\x31\xC0\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + memcpy_stack), 1, "\xEB", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 /*ret = proc_write_mem(ssc, (void *)(text_seg_base + ptrace2_patches), 1, "\xEB", &n);
	 if (ret)
	 {
		 goto error;
	 }*/
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + ptrace3_patches), 5, "\xE9\x7C\x02\x00\x00", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + aslr_patches), 1, "\xEB", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + C3_patches), 1, "\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sys_mount_unprivileged_user), 6, "\x90\x90\x90\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sys_dynlib_dlsym_patch2), 3, "\x31\xC0\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sys_map), 4, "\x37\x41\xB2\x37", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sys_dynlib_dlsym_patch3), 6, "\x90\x90\x90\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + Send_sysveri), 1, "\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + Fuck), 8, "\x48\xC7\xC0\x00\x00\x00\x00\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sceSblSysVeriInitialize), 8, "\x48\xC7\xC0\x00\x00\x00\x00\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sceVeri), 8, "\x48\xC7\xC0\x00\x00\x00\x00\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sceVeri_initialized), 1, "\0", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 /*ret = proc_write_mem(ssc, (void *)(text_seg_base + spoof), 4, "\x01\x80\x00\x09", &n);
	 if (ret)
	 {
		 goto error;
	 }*/
...
PAYLOAD_CODE void set_dipsw(int debug_patch) {
	...
	dipsw[0x36] = debug_patch ? 0x37 : 0x14;
	dipsw[0x59] = debug_patch ? 0x01 : 0x02;
